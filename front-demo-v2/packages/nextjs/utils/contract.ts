...
const deepMergeContracts = <L extends Record<PropertyKey, any>, E extends Record<PropertyKey, any>>(
  ...
  for (const key of allKeys) {
    ...
    const amendedExternal = Object.fromEntries(
      Object.entries(external[key] as Record<string, Record<string, any>>).map(([contractName, declaration]) => [
        contractName,
    ...
    onLogs: (
      logs: Simplify<
        Omit<Log<bigint, number, any>, "args" | "eventName"> & {
          args: Record<string, any>; // `unknown` yerine `any` kullanıldı
          eventName: string;
        }
      >[],
    ) => void;
...
type QueryClientConfig = Config;

export type ScaffoldQueryClientConfig = QueryClientConfig;
